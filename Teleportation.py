...
with st.expander("Как выбирается база (коротко и понятно)", expanded=False):
    st.markdown("""
**Шаг 1.** Для каждой базы врага считаем сумму расстояний до всех остальных баз гильдии.  

**Шаг 2.** Выбираем базу, у которой эта сумма минимальна.  

Это означает, что система выбирает **самую «центральную» базу внутри гильдии** — ту, которая в среднем ближе всего к остальным.

Расстояние считается:
- **Манхэттен** — по клеткам (|dx| + |dy|)
- **Евклид** — по прямой линии
""")
